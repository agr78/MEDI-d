...
%   wG added temporarily added as an output 2020.10.24

function [x, cost_reg_history, cost_data_history, wG, resultsfile] = MEDI_wavelets(varargin)
...
disp('Gradient mask generated.')

%ppb_thresh = 15;
%x_thresh = ppb_thresh*(2*pi*delta_TE*CF)*(1/(1e6.*Mask));
...
switch solver
    ...
    function [x, cost_reg_history, cost_data_history] = gaussnewton()
        ...
        Mask_shell = Mask-MaskErode(Mask,matrix_size,voxel_size,1);
        % ROI cheat for comparison
        %ROIs = readROIs('ROI/ROIs');
        %R = ~ROIs.img;
        ...
        while (res_norm_ratio>tol_norm_ratio)&&(iter<max_iter)
            ...
            %if x <= x_thresh
                reg_w = @(dx)wd_array(Mask_shell.*x,Mask_shell.*dx);
            %else
            %    reg_w = 0;
            %end
            ...
            A =  @(dx) reg_w(dx) + reg(dx) + 2*lambda*fidelity(dx);
            b = reg_w(x) + reg(x) + 2*lambda*Dconv( real(conj(w).*conj(1i).*(w-b0)) );
