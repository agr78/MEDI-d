...
[labeledImage, numROIs] = bwlabeln(imbinarize(imgaussfilt3(Mask.*N_std,round(3.*vox_min)),'adaptive'));
...
LT_reg_ROI = @(x) zeros(size(x));    
for j = 1:numROIs
    Mask_ROI{j} = labeledImage_Nstd == j;
    Mask_ROI{j}(Mask_ROI{j}>0) = 1;
    Mask_ROI{j} = logical(Mask_ROI{j});
    
    LT_reg_ROI = @(x) LT_reg_ROI(x) + Mask_ROI{j}.*(x-mean(x(Mask_ROI{j})));
   % if j == 1
   %    LT_reg_ROI = @(x) LT_reg_ROI(x) +...
   %         imbinarize(N_std,'adaptive').*imgaussfilt3(double(Mask_ROI{j}),...
   %         round(3.*vox_max)).*(x - mean(x(Mask_ROI{j})));
            
   % else
   %     LT_reg_ROI = @(x) LT_reg_ROI(x) + imbinarize(N_std,'adaptive')...
   %         .*Mask_ROI{j}.*(x - mean(x(Mask_ROI{j})));
   % end
...
Mask_s_ds = imerode(Mask_s_ds,strel('cube',double(round(3*vox_min))))...
    +(imresize3(double(Mask_ROI{1}),...
      [size(wG_ds,1) size(wG_ds,2) size(wG_ds,3)]));
...
wG_ds = wG_ds+imgaussfilt3(...
    imresize3(double((Mask_ROI{1})),[size(wG_ds,1) size(wG_ds,2) size(wG_ds,3)]),...
    double(round(3*vox_max)));
...
switch solver
    ...
    function [x, cost_reg_history, cost_data_history] = gaussnewton()
        ...
        while (res_norm_ratio>tol_norm_ratio)&&(iter<max_iter)
            ...
            reg_ROI = @(dx) LT_reg_ROI(LT_reg_ROI(real(dx)));
            reg = @(dx) reg(dx) + 15.*reg_ROI(dx);
