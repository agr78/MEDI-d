...
[lambda, ~, RDF, N_std, iMag, Mask, matrix_size, matrix_size0, voxel_size, ...
    ...
    Debug_Mode, lam_CSF, Mask_CSF, lam_Downsample, crop_factor, solver, percentage] = parse_QSM_input_extended_d(varargin{:});
...
if flag_DS
    fprintf('Downsampling regularization used\n');
end
...
switch solver
    ...
    function [x, cost_reg_history, cost_data_history] = gaussnewton()
        ...
        while (res_norm_ratio>tol_norm_ratio)&&(iter<max_iter)
            ...
            if flag_DS
                reg_ds = @(dx) real(k_pad(div(wG_ds.*Vr_ds.*(wG_ds.*grad(k_crop(real(dx),crop_factor),crop_factor*voxel_size)),crop_factor*voxel_size),end_size));
