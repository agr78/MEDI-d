...
function [lam, iFreq, RDF, N_std, iMag, Mask, matrix_size, matrix_size0, voxel_size, ...
    ...
    Debug_Mode, lam_CSF, Mask_CSF, lam_Downsample, crop_factor, solver, percentage] = parse_QSM_input_extended(varargin)
...
if size(varargin,2)>0
    for k=1:size(varargin,2)
        ...
        if strcmpi(varargin{k},'lam_Downsample')
            lam_Downsample = varargin{k+1};
        end
        if strcmpi(varargin{k},'crop_factor')
            crop_factor = varargin{k+1};
        end        
